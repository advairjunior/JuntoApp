...
public sealed class AceiteConviteDoGrupo(
    IRepositorioDeUsuarios repositorioDeUsuarios,
    IRepositorioDeGrupos repositorioDeGrupos,
    IRelogio relogio,
    IUnidadeDeTrabalho unidadeDeTrabalho)
{
    public async Task<ConviteDoGrupoRespondidoResposta> AceiteAsync(
    ...
    private static ConviteDoGrupo ObtenhaConvite(Grupo grupo, Guid identificadorDoConvite)
    {
        ConviteDoGrupo? convite = grupo.Convites.FirstOrDefault(conviteDoGrupo =>
            conviteDoGrupo.Identificador == identificadorDoConvite) ?? throw new UnauthorizedAccessException("Convite nao encontrado para o usuario atual.");
